...
Since we are assuming that both our API modules will perform the same actions and send data in the same unified format back to the GenServer, we can go one step further and not just assume, but require, that they do so. 
To this end, we can use another module to define the desired Behaviour and implement said Behaviour in our API modules. Now, we cannot only be sure that our existing API modules act a certain way, but we can also easily add further API modules in the future and then implement the same Behaviour for them, thus ensuring that we don't accidentally add a rogue API module that acts different from the rest and risks to break down its associated GenServer.
...
API module. This allows us to pattern match ont the api_module in our `handle_info` function and to pass any search params we might have around as part of the state as well.
